...
class Test():
    ...
    def compile(self,jlc):
        p=Popen([join('.',basename(jlc)),self.file], stdout=PIPE, stderr=PIPE,cwd=dirname(jlc))
...
class Summary():
   def analyzeGood(self,test):
	total=[0,0]
        if not self.shouldWork.has_key(test.category): self.shouldWork[test.category]=[0,0,0,[]]
   ...
   def __init__(self,tests,submission):
	...
	for test in tests:
	   ...
	   if test.category=="good" or "extension" in test.category:
	        ...
	        total=map(lambda x,y : x+y,total,subtotal)
	...
	self.good=self.shouldWork['good'] if self.shouldWork.has_key('good') else [0,0,0,[]]
	self.bad=bad
	self.total=total
